...
interface ProductDTO {    //Creo un interfaccia che è il metodo migliore di gestione dati di angular con le prorpietà dell'oggetto che mi interessa
  id: number;             // Recuperate dalla chiamata http GET e quindi dal JSON 
  title: string;
  price: number;
  image: string;
}
...
export class ProdcutsService {
  ...
  getProducts(): Observable<Product[]> {
    ...
    }).pipe(  // è un metodo che offre più step per manipolare le informazioni ricevute e trasformale come si vuole
  ...
  // private protective e public sono espressioni di visibilità 
  private getProductFromProductDTO(productDTO: ProductDTO):Product {   
    return {
      id: productDTO.id,
      name: productDTO.title,    // Conversione di proprietà scritte in una funzione che si riutilizzerà in .pipe , precisamente nella funzione RXjs "map"
